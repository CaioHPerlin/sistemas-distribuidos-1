...
As classes em `src/sensors/sensors.ts` são a fonte de verdade dos sensores e de suas unidades: temperatura em °C, umidade em % e chuva em mm/h. O talhão envia valores e unidades no contrato `SensorReading` de `src/sensors/protocol.ts`. O microsserviço define `AverageRequest` (`readings`) e `AverageResponse` (`averages`) em `src/microservice/protocol.ts`. O endereço `host:porta` da conexão identifica o talhão no gateway; não há IDs nas mensagens. O microsserviço apenas calcula médias, sem acessar o banco.
...
4. Em `src/microservice/protocol.ts`, acrescente `light: number` a `Averages`. Em `src/microservice/server.ts`, inclua `light: average(message.readings, 'light')` na resposta. O microsserviço continua calculando somente a média dos valores.
5. Em `src/gateway/main.ts`, inclua a média de `light` e a unidade recebida em `first.units.light` no log. A chamada de `database.save` já passa o objeto `units` completo.
6. Em `src/gateway/database.ts`, adicione as colunas `light` e `light_unit` à tabela `averages` e inclua os dois valores no `INSERT` de `save`. O banco de uma nova instância de gateway será criado com essas colunas.
7. Rode `npm run build` e inicie talhão, microsserviço e gateway com os comandos acima. Depois de formar um lote, consulte o SQLite com `SELECT light, light_unit FROM averages` e confirme os valores. Se estiver usando máquinas diferentes, atualize o código e compile em todas elas antes de iniciar os processos.
